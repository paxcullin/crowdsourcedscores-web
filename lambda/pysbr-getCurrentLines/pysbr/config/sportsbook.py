...
class Sportsbook(Config):
    ...
    def __init__(self):
        ...
        self._sportsbook_sys_ids = self._build_sportsbook_sys_ids()
    ...
    def _build_sportsbook_sys_ids(self) -> Dict[str, Dict[str, int]]:
        ...
        for k in ["name", "alias"]:
            sportsbooks[k] = {}
            for x in s:
                sportsbooks[k][x[k].lower()] = x["system sportsbook id"]
    ...
    def sysids(self, terms: Union[List[Union[int, str]], int, str]) -> List[int]:
        """Take provided search terms and return list of matching sportsbook ids.
