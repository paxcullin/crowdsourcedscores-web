    ...
    */
    ...
    try {
        ...
        if (profileUpdate.modifiedCount === 1) {
            ...
            console.log('profile', profile)
            session.endSession();
            context.done(null, { status: 200, message: `The balance of ${username} was updated successfully. ${currencyAmount * currencyMultiplier}`, currency: profile.currency, currencyHistory: {
                history: profile.currencyHistory.history.sort((a,b) => new Date(b.date) - new Date(a.date)) }
            })
        }
